...
void AGeometryHubActor::DoActorSpawn1()
{
	...
	if (World) {
		...
		//чтобы не кастовать AActro к нашему basegeometryactor вызовем функцию спавн по шаблону

		for (int32 i = 0; i < 10; ++i) {
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator,
				FVector(0.0f, (300.0f + (150 * i)), 300.0f));
...
void AGeometryHubActor::DoActorSpawn2()
{
	...
	if (World) {
		for (int32 i = 0; i < 10; ++i) {
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator,
				FVector(0.0f, (300.0f + (150 * i)), 700.0f));
			ABaseGeometryActor* Geometry = World->SpawnActorDeferred<ABaseGeometryActor>(GeometryClass,
				GeometryTransform);
			if (Geometry) {
				FGeometryData Data;
...
{
	if (!Actor) return;
	UE_LOG(LogGeometryHubActor, Error, TEXT("Timer finished %s"), *Actor->GetName());

	ABaseGeometryActor* Geometry = Cast<ABaseGeometryActor>(Actor);
	if (!Geometry) return;
	UE_LOG(LogGeometryHubActor, Display, TEXT("cast and Amplitude %s"), *Geometry->GetAmplitudeToString());

	Geometry->Destroy();
	//Geometry->SetLifeSpan(2.0f);

}
////////////////Печать всех полей данной структуры сделать функцию
// ///////////// Функцию toString в GeometryData чтобы она печатала все поля структуры 
